...
function render(){
  ...
  for(let m of pills){
    if(assets.pill) ctx.drawImage(assets.pill, m.x, m.y, m.w, m.h);
    // draw small "Ahmed Taha" text above (pixel font)
    ctx.fillStyle = '#000'; ctx.font = '10px "Press Start 2P", monospace'; ctx.textAlign='center';
    ctx.fillText('Ahmed Taha', m.x + m.w/2, m.y - 6);
    // blink overlay occasionally (simulate blink by drawing a small rectangle)
    if(m.blinkTimer < 0.15){
      ctx.fillStyle = '#ffccff'; ctx.fillRect(m.x + 18, m.y + 18, 12, 6);
    }
  }
